...
int dis_env(__attribute__((unused)) char **cmd, __attribute__((unused)) int st)
{
    ...
    {
        len = _strlen(environ[i]);  // Get the length of the environment variable string
...
{
    char *filename = "../test/simple_shell_history";  // Name of the history file
